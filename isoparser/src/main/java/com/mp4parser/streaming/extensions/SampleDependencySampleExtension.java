...
public class SampleDependencySampleExtension implements TrackExtension {
    public static Map<Byte, SampleDependencySampleExtension> pool =
            Collections.synchronizedMap(new HashMap<Byte, SampleDependencySampleExtension>());
    private byte d;

    public static SampleDependencySampleExtension create(byte d) {
        SampleDependencySampleExtension c = pool.get(d);
        if (c == null) {
            c = new SampleDependencySampleExtension();
            c.d = d;
            pool.put(d, c);
        }
        return c;
