...
public final class MediaDataBox<T extends TrackMetaDataContainer> extends Box {
  ...
  public void parse(IsoInputStream in, long size, BoxFactory boxFactory, Box lastMovieFragmentBox) throws IOException {
    ...
    startOffset = in.getStreamPosition();
    sizeIfNotParsed = size;
    int paranoid = 0;

    while (size > 0) {
      size -= in.skip(size);
      // Because "for a variety of reasons" skip might skip less bytes than expected
      // we need some mechanism to break out of an endless loop. Here it is:
      if (++paranoid == 13) {
  ...
  public void parseTrackChunkSample() throws IOException {
    if (!contentsParsed) {
      ...
      RandomAccessDataSource raf = getIsoFile().getOriginalIso();
      ...
      trackBoxContainer.parseMdat(this, raf);
